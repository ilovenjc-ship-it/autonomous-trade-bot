...
| **Naming sweep — TaoBot → Ari (forward-looking refs only)** | Low (queued Day 13) | Mark's directive 2026-05-26: "We can't use the name TaoBot, it's already taken as a Tao stats validator." Repo audit (Day 13) found 35 hits across STATE.md / SUCCESSOR_BRIEF.md / docs/discord-onboarding/bittensor-server-onboarding.md / report/SESSION_XI_ARCHIVE.md. **Three buckets:** (A) ~13 historical/factual refs DO NOT change — TaoStat's actual validator hotkey `5E2LP6…Z5u` in SESSION_XI_ARCHIVE, transparency notes recording the Day-7 OTF-Signal-Bot misread, shipped frontend localStorage keys `taobot:sidebar:expanded-groups:v1` / `taobot:sidebar:user-default:v1` (renaming silently breaks every user's sidebar state — needs migration shim, not find/replace). (B) ~22 forward-looking project-name refs DO change — STATE.md "Day 7 Reading List" relevance verdicts (lines ~1615, 1814, 2735, 3048–3055, 3067–3074, 3083, 3095, 3104, 3134, 3166, 3198, 3228, 3275, 3278, 3313) + SUCCESSOR_BRIEF.md line 265. Replace "TaoBot" → "Ari" or "the Project" / "the App" depending on whether the sentence is talking about the intelligent layer or the product. (C) RESEARCH_LOG.md row 1 (Zyfai "managed TaoBot SDK") already fixed Day 13 commit `540dd206`. **Default plan:** Option 2 from Day 13 conversation — sweep Bucket B + add a one-line "naming superseded" note at top of STATE.md so future Ari understands why historical "TaoBot" mentions in archives aren't bugs. **Do not touch** Bucket A's localStorage keys without an explicit migration plan (would break shipped user state on next page load). ETA when scheduled: ~10 min.  |
