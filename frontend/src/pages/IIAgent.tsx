...
const REGIME_CONFIG: Record<string, { icon: typeof TrendingUp; label: string; glow: string; bg: string; text: string }> = {
  ...
  UNKNOWN:  { icon: Activity,     label: 'SCANNING…',     glow: 'shadow-slate-500/20',   bg: 'bg-slate-700/30 border-slate-600/40',     text: 'text-slate-400'   },
}
...
const HEALTH_CONFIG: Record<string, { label: string; border: string; bg: string; text: string; dot: string }> = {
  ...
  INACTIVE:   { label: '⚙️ WARMING',  border: 'border-slate-600/40',   bg: 'bg-slate-700/30',   text: 'text-slate-500',   dot: 'bg-slate-600'   },
}
    ...
    <div className={clsx(
      ...
      <div className="flex items-center justify-between relative">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-mono">Market Regime</p>
      ...
      <div className="flex gap-4 relative">
        <div>
          <p className="text-[10px] text-slate-500 font-mono">TAO Price</p>
          <p className="text-lg font-bold text-white font-mono">${price?.toFixed(2) ?? '—'}</p>
        </div>
        {rsi !== null && (
          <div>
            <p className="text-[10px] text-slate-500 font-mono">RSI-14</p>
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-3 mt-1">
            ...
            <span className="text-[10px] text-slate-600 font-mono">{timeSince(obs.timestamp)}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className={clsx('rounded-xl border p-4 flex gap-3', cfg.bg, cfg.border)}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          {rec.strategy && (
            <span className="text-[10px] text-slate-500 font-mono truncate">{rec.strategy}</span>
          )}
        </div>
        <p className="text-xs text-slate-300 leading-relaxed">{rec.action}</p>
        <p className="text-[10px] text-slate-600 font-mono mt-1">{timeSince(rec.timestamp)}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-mono">
              ...
              {status?.last_analysis_at && (
                <span className="ml-2 text-slate-600">· Last analysis {timeSince(status.last_analysis_at)}</span>
              )}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-600 font-mono">↻ {lastRefresh.toLocaleTimeString()}</span>
          <button
          ...
          <button onClick={load} className="p-2 rounded-lg bg-dark-700 border border-dark-600 text-slate-400 hover:text-white transition-colors">
            <RefreshCw size={14} />
          </button>
        </div>
      </div>
      ...
      {fleetBots.length > 0 && (
        <div className="bg-dark-800 border border-dark-600 rounded-2xl p-4 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500 uppercase tracking-wider font-mono">Fleet Health Monitor</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-dark-800 border border-dark-600 rounded-2xl overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-dark-700 flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500 uppercase tracking-wider font-mono">Agent Observation Log</span>
            <span className="ml-auto text-[10px] text-slate-600 font-mono">{observations.length} entries</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto max-h-[420px] divide-y divide-dark-700/50 px-4 py-2 space-y-0">
            {observations.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <p className="text-slate-600 text-sm font-mono">Agent initialising…</p>
        ...
        <div className="bg-dark-800 border border-dark-600 rounded-2xl overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-dark-700 flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500 uppercase tracking-wider font-mono">Directives</span>
            <span className="ml-auto text-[10px] text-slate-600 font-mono">{recommendations.length}</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto max-h-[420px] p-3 space-y-2">
            {recommendations.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <p className="text-slate-600 text-xs font-mono">No recommendations yet.</p>
              </div>
            ) : (
      ...
      <div className="bg-dark-800/60 border border-dark-700 rounded-xl p-4">
        <p className="text-xs text-slate-500 uppercase tracking-wider font-mono mb-3">Orchestration Architecture</p>
        <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
          {[
            { label: '🧠 II Agent', sub: 'orchestrator', color: 'text-indigo-400', bg: 'bg-indigo-500/10 border-indigo-500/30' },
            { label: '→', sub: '', color: 'text-slate-600', bg: '' },
            ...
            <div key={i} className={clsx('px-3 py-1.5 rounded-lg border', item.bg)}>
              ...
              {item.sub && <p className="text-slate-600 text-[9px]">{item.sub}</p>}
            </div>
          ) : (
